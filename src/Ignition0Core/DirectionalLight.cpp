...
void DirectionalLight::updateUBO(const int index) {
	if(STATE & (ROTATION_CHANGED | PROPERTY_CHANGED)) {
		Material0::DirectionalLightProperties data = {
			glm::vec4(Direction,0),
			glm::vec4(Color,Intensity)
		};

		glBufferSubData(GL_UNIFORM_BUFFER, (sizeof(int) * 4) + (sizeof(Material0::DirectionalLightProperties) * index), sizeof(Material0::DirectionalLightProperties), &data);
	}
}